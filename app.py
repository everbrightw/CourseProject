...
def set_sess(url, ses_disp_str):
    ...
    if (len(session['urls']) == 0) or (
            len(session['urls']) > 0 and session['urls'][-1] != url):  # consecutive visits to same slide
...
def slide(course_name, lno):
    ...
    disp_snippet), lec_names, lnos, ses_disp_str = resolve_slide(course_name, lno, 'drop-down')
    vis_urls, vis_strs = get_prev_urls()

    if next_slide_name is not None:
        set_sess(request.url, ses_disp_str)

    return render_template("slide.html", slide_name=next_slide_name, course_name=course_name,
                           num_related_slides=num_related_slides, related_slides=related_slides, disp_str=disp_str,
                           disp_color=disp_color, disp_snippet=disp_snippet, related_course_names=related_course_names,
                           lno=lno, lec_name=lec_name, lec_names=lec_names, lnos=lnos, course_names=COURSE_NAMES,
                           num_courses=NUM_COURSES, rel_lnos=rel_lnos, rel_lec_names=rel_lec_names, vis_urls=vis_urls,
                           vis_strs=vis_strs, num_vis=NUM_VIS)
...
def related_slide(course_name, slide_name, lno):
    ...
    next_slide_name, lno, lec_name, (
    num_related_slides, related_slides, disp_str, related_course_names, rel_lnos, rel_lec_names, disp_color,
    disp_snippet), lec_names, lnos, ses_disp_str = resolve_slide(course_name, lno, 'related', slide_name=slide_name)
    vis_urls, vis_strs = get_prev_urls()
...
def next_slide(course_name, lno, curr_slide):
    ...
    next_slide_name, lno, lec_name, (
    num_related_slides, related_slides, disp_str, related_course_names, rel_lnos, rel_lec_names, disp_color,
    disp_snippet), lec_names, lnos, ses_disp_str = resolve_slide(course_name, lno, 'next', slide_name=curr_slide)
...
def prev_slide(course_name, lno, curr_slide):
    global NUM_VIS
    prev_slide_name, lno, lec_name, (
    num_related_slides, related_slides, disp_str, related_course_names, rel_lnos, rel_lec_names, disp_color,
    disp_snippet), lec_names, lnos, ses_disp_str = resolve_slide(course_name, lno, 'prev', slide_name=curr_slide)
    ...
    if prev_slide_name is not None:
        return render_template("slide.html", slide_name=prev_slide_name, course_name=course_name,
                               num_related_slides=num_related_slides, related_slides=related_slides, disp_str=disp_str,
                               disp_color=disp_color, disp_snippet=disp_snippet,
                               related_course_names=related_course_names, lno=lno, lec_name=lec_name,
                               lec_names=lec_names, lnos=lnos, course_names=COURSE_NAMES, num_courses=NUM_COURSES,
...
def search_results(course_name=None, lno=None, slide_name=None, curr_slide=None):
    ...
    num_results, results, disp_strs, search_course_names, lnos, snippets, lec_names = model.get_search_results(
        search_string)
    if not results:
        num_results = 0
        results = []
    response = jsonify({'num_results': num_results, 'results': results, 'disp_strs': disp_strs,
                        'search_course_names': search_course_names, 'lnos': lnos, 'course_names': COURSE_NAMES,
                        'num_courses': NUM_COURSES, 'snippets': snippets, 'lec_names': lec_names
                        })
    return response
...
def log_helper(action, route):
    if action is not None and route is not None:
        ...
        if func_type == 'related_slide':
            resolve_slide(route_ele[beg], route_ele[beg + 1], 'related', slide_name=route_ele[beg + 2], log=True,
                          action=action)
        elif func_type == 'next_slide':
            resolve_slide(route_ele[beg], route_ele[beg + 1], 'next', slide_name=route_ele[beg + 2], log=True,
                          action=action)
        elif func_type == 'prev_slide':
            resolve_slide(route_ele[beg], route_ele[beg + 1], 'prev', slide_name=route_ele[beg + 2], log=True,
                          action=action)
            ...
            resolve_slide(route_ele[beg], route_ele[beg + 1], 'search_results', slide_name=route_ele[beg + 2], log=True,
                          action=action)
